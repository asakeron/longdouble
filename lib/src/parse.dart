...
final RegExp _LD_REGEXP = new RegExp(r"(\+|-)?(\d*.)?\d+(e|E(\+|-)?\d+)?");
final RegExp _INF_REGEXP = new RegExp(r"(\+|-)?Infinity");
final RegExp _NAN_REGEXP = new RegExp(r"(\+|-)?NaN");
...
const List<longdouble> _NEG_POWERS_OF_TEN = const [
  const longdouble.zero(),
...
longdouble _parseLongdouble(String source,
    [longdouble onError(String source)?]) {
  ...
  if (exponent >= 0) {
    ...
    exponentMultiplier = math_ld.intpow(new longdouble(10.0), exponent);
  } else if (exponent < 0) {
    ...
    if (exponent >= -22) {
      ...
      exponentMultiplier = _NEG_POWERS_OF_TEN[22] *
          math_ld.intpow(new longdouble(10.0), exponent + 22);
    }
  }
